...
def show_checkout_screen():
    ...
    with col1:
        ...
        # ★重要: keyに変数を使うことで、IDが変わるとカメラが作り直されます
        ...
        if img_file_buffer is not None:
            ...
            if decoded_objects:
                for obj in decoded_objects:
                    ...
                    # ★ここが修正のキモです★
                    # 1. カメラのID番号を増やして、次の実行時に「新しいカメラ」として読み込ませる
                    st.session_state['camera_key_id'] += 1
                    
                    # 2. ここで明示的に再実行し、画面とカメラをリフレッシュする
                    st.rerun()
                    
                    break 
            else:
                st.warning("バーコードが検出されませんでした。もう一度試してください。")
                # 検出できなくても写真をクリアしたい場合は、ここにもClearボタンなどを置くことができます
